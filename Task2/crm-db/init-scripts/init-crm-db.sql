...
-- Таблица данных миодатчиков
CREATE TABLE IF NOT EXISTS emg_sensor_data (
                                               user_id INTEGER,
                                               prosthesis_type VARCHAR,
...
COPY emg_sensor_data (user_id, prosthesis_type, muscle_group, signal_frequency, signal_duration, signal_amplitude, signal_time)
    FROM '/docker-entrypoint-initdb.d/olap.csv'
    DELIMITER ','
    CSV HEADER;